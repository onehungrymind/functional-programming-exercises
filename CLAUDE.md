...
- **A prompt names every export the learner has to write, and says what it returns.** Not
  "write it and the other one": name each, in the order they depend on each other, and for
  anything that reports an observation rather than computing a value, say exactly what makes it
  true. A starter comment is not a substitute, because the prompt is what gets read first.
  Scaffolding handed over finished is exempt. Verify enforces this; `npx tsx
  scripts/audit-clarity.mjs` reports the softer signals it cannot gate on.
...
| `npm run build:solutions` | Regenerate `solutions.html`. Committed, and verify fails if it is stale. |
| `npm run build:content` | Both of the above. Use this after changing an exercise. |
| `npx tsx scripts/audit-clarity.mjs` | Reports rungs whose directions are probably unclear. A report, not a gate. |
| `npx tsx scripts/probe-fatals.mjs` | Reports broken variants that fail by crashing rather than by failing a check. |
